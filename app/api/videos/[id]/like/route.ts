...
async function likeCount(env: Env, videoId: number): Promise<number> {
  const row = await env.DB.prepare(
    "SELECT COUNT(*) AS c FROM likes WHERE video_id = ?"
  )
    .bind(videoId)
    .first<{ c: number }>();
  return row?.c ?? 0;
...
export async function POST(_request: Request, { params }: Params) {
  ...
  return json({ likes: await likeCount(env, videoId) });
}
